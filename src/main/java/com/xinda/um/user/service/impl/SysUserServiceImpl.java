...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Override
    public SysUser validateLoginInfo(SysUser sysUser) throws LoginException {
        // 校验登录信息
        if (StringUtils.isEmpty(sysUser.getUserName()) || StringUtils.isEmpty(sysUser.getPassword())) {
            throw new LoginException(LoginException.MSG_ERROR_SYS_USER_NAME_NOT_NULL);
        }
        SysUser user = sysUserMapper.querySysUserBySysUser(sysUser);
        if (null == user) {
            throw new LoginException(LoginException.MSG_ERROR_SYS_USER_NOT_EXISTS);
        }
        if (!"Y".equals(user.getStatus())) {
            throw new LoginException(LoginException.MSG_ERROR_SYS_USER_NAME_INVALID);
        }
        if (!sysUser.getUserName().equals(user.getUserName())) {
            throw new LoginException(LoginException.MSG_ERROR_SYS_USER_NAME_ERROR);
        }
        if (!encryptionService.encode(sysUser.getPassword()).equalsIgnoreCase(user.getPassword())) {
            throw new LoginException(LoginException.MSG_ERROR_SYS_USER_PASSWORD_ERROR);
        }
        return user;
    }
    ...
    public SysUser createSysUser(SysUser sysUser) throws LoginException {
        //加密存储
        ...
        if (null == sysUser.getUserId()) {//insert
            ...
            if (null != sysUserMapper.querySysUserBySysUser(user)) {//存在同名的用户名
                ...
                throw new LoginException(LoginException.MSG_ERROR_SYS_USER_NAME_HAD_EXISTS);
            }
            ...
            if (!user.getPassword().equals(encryptionService.encode(sysUser.getOldPassword()))) {
                ...
                throw new LoginException(LoginException.MSG_ERROR_SYS_USER_OLD_PASSWORD_NOT_EQUALS);
            }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public List<SysUser> batchSaveOrUpdateSysUsers(List<SysUser> sysUsers) throws LoginException {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public List<SysUser> deleteSysUsers(List<SysUser> sysUsers) throws LoginException {
        for (SysUser sysUser : sysUsers) {
